#ifndef QR_PARALELNA_BLOKOVSKA_H
#define QR_PARALELNA_BLOKOVSKA_H
...
namespace ParallelBlockedQR {
	...
	public:
		...
		// Calculates and returns transpose of a matrix (OpenMP parallelized)
...
#pragma omp parallel for
	...
	// Function declaration for external use by dgemm_parallel (OpenMP parallelized)
	...
	public:
		...
		// Helper functions (already parallelized or don't benefit from parallelization)
...
#pragma omp parallel for
			...
			for (int k = n - 1; k >= 0; --k) {
				...
				// Parallelization of H_k application across columns (j)
#pragma omp parallel for
				for (int j = 0; j < n_Q; ++j) {
		...
		// Error functions are already parallelized
...
#endif // QR_PARALELNA_BLOKOVSKA_H