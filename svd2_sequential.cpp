...
public:
  ...
  Matrix(const Matrix &other)
      : rows(other.rows), cols(other.cols), stride(other.stride) {
...
// ============================================================================
// OPTIMIZED C++ IMPLEMENTATION (Sequential)
// ============================================================================
...
double householderVector(double *RESTRICT x, int n, double &v0_out) {
  ...
  for (; i <= n - 8; i += 8) {
    ...
    v_norm_sq += x[i + 4] * x[i + 4];
    v_norm_sq += x[i + 5] * x[i + 5];
    v_norm_sq += x[i + 6] * x[i + 6];
    v_norm_sq += x[i + 7] * x[i + 7];
  }
  for (; i < n; ++i)
    v_norm_sq += x[i] * x[i];
...
void applyHouseholderLeft(Matrix &A, int rowStart, int colStart,
  ...
  for (int j = 0; j < w_len; ++j) {
    ...
    double sum = A_col[0]; // v[0] = 1.0 implicitly
    int i = 1;
    for (; i <= v_len - 8; i += 8) {
      sum += A_col[i] * v_ptr[i];
      ...
      sum += A_col[i + 4] * v_ptr[i + 4];
      sum += A_col[i + 5] * v_ptr[i + 5];
      sum += A_col[i + 6] * v_ptr[i + 6];
      sum += A_col[i + 7] * v_ptr[i + 7];
    }
    for (; i < v_len; ++i)
      sum += A_col[i] * v_ptr[i];
    work_ptr[j] = sum;
  ...
  for (int j = 0; j < w_len; ++j) {
    ...
    for (; i <= v_len - 8; i += 8) {
      ...
      A_col[i + 4] -= v_ptr[i + 4] * val;
      A_col[i + 5] -= v_ptr[i + 5] * val;
      A_col[i + 6] -= v_ptr[i + 6] * val;
      A_col[i + 7] -= v_ptr[i + 7] * val;
    }
    for (; i < v_len; ++i)
      A_col[i] -= v_ptr[i] * val;
  }
...
void applyHouseholderRight(Matrix &A, int rowStart, int colStart,
  ...
  for (int j = 1; j < v_len; ++j) {
    ...
    i = 0;
    for (; i <= w_len - 4; i += 4) {
      A_col[i] -= work_ptr[i] * factor;
      A_col[i + 1] -= work_ptr[i + 1] * factor;
      A_col[i + 2] -= work_ptr[i + 2] * factor;
      A_col[i + 3] -= work_ptr[i + 3] * factor;
    }
    for (; i < w_len; ++i)
      A_col[i] -= work_ptr[i] * factor;
  }
...
// Main Algorithm
// ------------------------------------------------------------------------
void compute(const Matrix &inputA, Matrix &U, std::vector<double> &S,
             Matrix &V);
...
void compute(const Matrix &inputA, Matrix &U, std::vector<double> &S,
  ...
  while (iter < max_iter) {
    ...
    int m_idx = min_mn - 1;
    while (m_idx > 0) {
      if (std::abs(f[m_idx - 1]) <=
          EPSILON * (std::abs(d[m_idx - 1]) + std::abs(d[m_idx]))) {
        f[m_idx - 1] = 0.0;
        break;
      }
      m_idx--;
    ...
    while (p >= 0) {
      if (p == 0)
        break;
      if (std::abs(f[p - 1]) <=
          EPSILON * (std::abs(d[p - 1]) + std::abs(d[p]))) {
        f[p - 1] = 0.0;
        break;
      }
      p--;
  ...
  for (int i = 0; i < (int)S.size() - 1; ++i) {
    ...
    if (max_idx != i) {
      ...
      // Swap columns of U using temp buffer
  ...
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      ...
      for (int k = 0; k < min_dim; ++k) {
        // U(:,k) * S[k] * V(j,k)
        reconstructed += U.col_ptr(k)[i] * S[k] * V.col_ptr(k)[j];
      }
      double diff = A_copy.col_ptr(j)[i] - reconstructed;
      error_sq += diff * diff;
    }
  }
  double recon_error = std::sqrt(error_sq);

  // Also compute ||A||_F for relative error
  double a_norm_sq = 0.0;
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      double val = A_copy.col_ptr(j)[i];
      a_norm_sq += val * val;
    }
  }
  double a_norm = std::sqrt(a_norm_sq);

  std::cout << "  Reconstruction Error (absolute): " << std::scientific
            << recon_error << "\n";
  std::cout << "  Reconstruction Error (relative): " << std::scientific
            << recon_error / a_norm << "\n";
}
